...
u8 DHT22_Read_Bit(void) 			 
{
 	u8 retry=0;
	u8 flag = 0;
	while(DHT22_DQ_IN&&retry<150)//等待变为低电平
	{
		retry++;
//		printf("aa%daa", DHT22_DQ_IN);
		delay_us(1);
	}
	retry=0;
	while(!DHT22_DQ_IN&&retry<100)//等待变高电平
	{
//				printf("cc%dcc", DHT22_DQ_IN);
		retry++;
		delay_us(1);
	}
	delay_us(40);//等待40us
...
// 	RCC_APB2PeriphClockCmd(RCC_AHB1Periph_GPIOB | RCC_APB2Periph_AFIO, ENABLE);	 //使能PB端口时钟
...
////	 GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NULL;
