    ...
    def _removeFix(self,node):
        ...
        while True:
            if node.color == 'RED' and flag:
                ...
                print('Case1')
                break
            elif node.getBro().color == 'RED' and flag:
                node.parent.setCol('RED')
                node.getBro().setCol('BLACK')
                try:
                    self._lrotation(node.parent)
                except Exception:
                    pass
                print('Case2')
            elif node.getBro().rchild.color == 'BLACK' and node.getBro().lchild.color == 'BLACK' and flag:
                bre = False
                if node.parent.color == 'RED':
                    bre = True
                node.parent.setCol('BLACK')
                node.getBro().setCol('RED')
                print('Case3')
                if bre:
                    break
            elif node.getBro().lchild.color == 'RED' and node.getBro().rchild.color == 'BLACK':
                node.getBro().setCol('RED')
                node.getBro().lchild.setCol('BLACK')
                try:
                    self._rrotation(node.getBro())
                except Exception:
                    pass
                flag = False
                print('Case4')
            elif node.getBro().rchild.color == 'RED':
                node.getBro().setCol(node.parent.color)
                node.getBro().rchild.setCol('BLACK')
                node.parent.setCol('BLACK')
                try:
                    self._lrotation(node.parent)
                except Exception:
